...
public class GUnit {
    public static void main(String[] args) throws IOException {
        ...
        String validLineRegex = String.format("%1$s%2$s%1$s%2$s%1$s",
                validNameElementRegex, validDelimiterRegex);
        ...
        classes.entrySet().stream()
                .sorted((clazz1, clazz2) -> {
                    int amountUnitTestsClazz1 = clazz1.getValue().values().stream()
                            .mapToInt(m -> m.size()).sum();

                    int amountUnitTestsClazz2 = clazz2.getValue().values().stream()
                            .mapToInt(m -> m.size()).sum();
                    ...
                    clazz.getValue().entrySet().stream()
                            .sorted((method1, method2) -> {
                                ...
                                result.append(String.format("##%s", method.getKey()))
                                        .append(System.lineSeparator());

                                method.getValue().stream()
                                        .sorted((unitTest1, unitTest2) -> {
                                            int comparatorByNamesLength = unitTest1.length() -
                                                    unitTest2.length();
                                        ...
                                        .forEach(unitTest -> {
                                            result.append(String.format("####%s", unitTest))
                                                    .append(System.lineSeparator());
                                        });
                    });
        });
